...
def initialize_exl():
    ...
    if os.path.exists(user_data_path): workbook = load_workbook(user_data_path)
    else: workbook = Workbook(); del workbook["Sheet"]
    if "contacts" not in workbook.sheetnames: workbook.create_sheet("contacts")
    worksheet = workbook["contacts"]
    for position, value in {"A1":"user_id", "B1":"first_name", "C1":"last_name", "D1":"phone_number"}.items(): worksheet[position] = value
    workbook.save(user_data_path)
...
def on_contact_send(message):
    ...
    for cell in worksheet.iter_rows(min_row = 2, max_col = 1):
        if cell[0].value == contact.user_id: insert_row = cell[0].row
    for letter, value in zip(["A", "B", "C", "D"], [contact.user_id, contact.first_name, contact.last_name, contact.phone_number]):
        worksheet[f"{letter}{insert_row}"] = value
    workbook.save(user_data_path)
...
if __name__ == "__main__":
    user_data_path = "Data\\UserData.xlsx"
    ...
    Thread(target = command_prompt.cmdloop).start()
